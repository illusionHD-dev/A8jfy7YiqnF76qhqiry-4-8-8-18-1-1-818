...
return function(ctx)
	...
	local fix
	local aimpart
	-- Internal camera/visibility rays must never re-enter the aim transforms.
	...
	use = patch:option('toggle', {
		name = 'Use Hitboxes',
		default = false,
		tooltip = 'Uses HitBoxes expansion for target acquisition. Aim Part controls the actual part aimed at and hit.'
	})
	...
	aimpart = patch:option('dropdown', {
		name = 'Aim Part',
		list = {'Head', 'Body', 'Torso', 'Random'},
		default = 'Head',
		tooltip = 'Head: head only. Body: center mass. Torso: upper torso/chest. Random: one available body part per targeting call.'
	})
	if not aimpart then
		ctx.log:add('patch', 'SilentAimfix', 'Aim Part dropdown could not be created')
		return
	end
	...
	local function bypass(origin, dir)
		...
		local short = math.clamp(zoom + 8, 10, 96)
		-- Unknown callers only bypass casts which actually end at a camera anchor.
		-- The old 8-stud/large-radius test also swallowed unit weapon rays.
		if len > 1.01 and len <= short then
			if near(last, pos, 1.5) and (near(origin, focus, 2) or sub and near(origin, sub, 2) or rootpos and near(origin, rootpos, 2)) then return true end
			if near(origin, pos, 1.5) and (near(last, focus, 1.5) or sub and near(last, sub, 1.5)) then return true end
		end
		return false
	end
	...
	local function entitypart(ent, name)
		...
		local part = ent[name]
		if typeof(part) == 'Instance' and part:IsA('BasePart') and part.Parent ~= nil then return part end
	...
	local function firstpart(ent, names)
		for _, name in ipairs(names) do
	...
	local function targetpart(ent, mode)
		if mode == 'Head' then return entitypart(ent, 'Head') end
		if mode == 'Torso' then
			return firstpart(ent, {'UpperTorso', 'Torso', 'LowerTorso', 'Body'})
		end
		if mode == 'Random' then
			local parts, seen = {}, {}
			for _, name in ipairs({'Head', 'UpperTorso', 'Torso', 'LowerTorso', 'Body',
		...
		-- Prefer physical body parts over the usually invisible root collider.
		return firstpart(ent, {'LowerTorso', 'Torso', 'UpperTorso', 'Body', 'RootPart'})
	end
	...
	local function mouse(sett, resolve, amount, active)
		...
		for _, ent in lib.List do
			...
			if not ent.Targetable or ent.Player == lp then continue end
			local part = resolve(ent)
			...
			if mag > (tonumber(sett.Range) or math.huge) then continue end
	...
	local function position(sett, resolve, amount, active)
		if type(lib) ~= 'table' or lib.isAlive == false or type(lib.List) ~= 'table' then table.clear(sett) return end
	...
	local function call(cur, args, quiet, capture)
		...
		local oldm, oldp = lib.EntityMouse, lib.EntityPosition
		local mode = aimpart and aimpart.Value or 'Head'
		local amount, active = 0, false
		if use and use.Enabled then
			local _, expand, enabled = data()
			amount, active = expand or 0, enabled == true
		end
		local selected = {}
		local function resolve(ent)
			if selected[ent] == nil then selected[ent] = targetpart(ent, mode) or false end
			return selected[ent] or nil
		end
		local function record(selector)
			return function(sett)
				local ent = selector(sett, resolve, amount, active)
				local part = ent and resolve(ent)
				if not part then return end
				if type(capture) == 'table' then
					capture.entity, capture.part = ent, part
				end
				-- Feed the exact selected part into the original hook's prediction
				-- and result construction, regardless of its head/root branch.
				-- Do not mutate the shared entity or override Hit Chance/AutoFire.
				return setmetatable({Head = part, RootPart = part, HumanoidRootPart = part}, {__index = ent})
			end
		end
		lib.EntityMouse, lib.EntityPosition = record(mouse), record(position)
		local out = table.pack(pcall(invoke, cur, args, quiet))
		lib.EntityMouse, lib.EntityPosition = oldm, oldp
		if not out[1] then error(out[2], 0) end
		return table.unpack(out, 2, out.n)
	end

	local function hintpart(params, ent)
		if typeof(params) ~= 'RaycastParams' or type(ent) ~= 'table' or params.FilterType ~= Enum.RaycastFilterType.Include then return end
		local char = ent.Character
		if typeof(char) ~= 'Instance' then return end
	...
	local function pickpart(ent, origin, dir, hinted)
		...
		local seen = {}
		local hname = data()
		for _, name in ipairs({hname or '', 'Head', 'UpperTorso', 'Torso', 'LowerTorso', 'RootPart', 'HumanoidRootPart'}) do
	...
	local function wrapraycast(cur)
		return function(args)
			...
			if capture.entity and typeof(origin) == 'Vector3' and typeof(args[2]) == 'Vector3' then
				local hint = capture.part or hintpart(args[3], capture.entity)
	...
	local function wraplegacy(cur)
		return function(args)
			...
			if capture.entity and typeof(beam) == 'Ray' and typeof(origin) == 'Vector3' then
				local pos, dir = transformvector(origin, beam.Direction, capture.entity, capture.part, state)
	...
	local function wrapcamera(cur)
		return function(args)
			...
			if capture.entity and type(out[1]) == 'table' and typeof(out[1][1]) == 'Ray' then
				local beam = out[1][1]
				local part = pickpart(capture.entity, beam.Origin, beam.Direction, capture.part)
	...
	local function wrapray(cur)
		return function(args)
			...
			if capture.entity and typeof(origin) == 'Vector3' and typeof(args[2]) == 'Vector3' then
				local pos, dir = transformvector(origin, args[2], capture.entity, capture.part, state)
				if pos then
					args[1] = pos
					args[2] = dir
				end
	...
	for name, val in pairs(hooks) do
		...
		local transform = wrap
		wrap = function(args)
			local thread = coroutine.running()
			if internal[thread] then return end
			internal[thread] = true
			local out = table.pack(pcall(transform, args))
			internal[thread] = nil
			if not out[1] then error(out[2], 0) end
			return table.unpack(out, 2, out.n)
		end
